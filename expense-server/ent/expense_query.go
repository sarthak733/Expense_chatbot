...
func (eq *ExpenseQuery) QueryUser() *UserQuery {
...
// QueryCategoryRef chains the current query on the "category_ref" edge.
func (eq *ExpenseQuery) QueryCategoryRef() *CategoryQuery {
...
func (eq *ExpenseQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (eq *ExpenseQuery) Clone() *ExpenseQuery {
	...
	return &ExpenseQuery{
		...
		withCategoryRef: eq.withCategoryRef.Clone(),
...
func (eq *ExpenseQuery) WithUser(opts ...func(*UserQuery)) *ExpenseQuery {
	query := (&UserClient{config: eq.config}).Query()
...
// WithCategoryRef tells the query-builder to eager-load the nodes that are connected to
// the "category_ref" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *ExpenseQuery) WithCategoryRef(opts ...func(*CategoryQuery)) *ExpenseQuery {
	query := (&CategoryClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withCategoryRef = query
	return eq
}
...
func (eq *ExpenseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Expense, error) {
	...
	if query := eq.withCategoryRef; query != nil {
		if err := eq.loadCategoryRef(ctx, query, nodes, nil,
			func(n *Expense, e *Category) { n.Edges.CategoryRef = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *ExpenseQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Expense, init func(*Expense), assign func(*Expense, *User)) error {
...
func (eq *ExpenseQuery) loadCategoryRef(ctx context.Context, query *CategoryQuery, nodes []*Expense, init func(*Expense), assign func(*Expense, *Category)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Expense)
	for i := range nodes {
...
func (eq *ExpenseQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withUser != nil {
			_spec.Node.AddColumnOnce(expense.FieldUserID)
		}
		if eq.withCategoryRef != nil {
			_spec.Node.AddColumnOnce(expense.FieldCategoryID)
		}
	}
