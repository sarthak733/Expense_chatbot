...
type CategoryQuery struct {
	...
	withRecurringExpenses *RecurringExpenseQuery
...
func (cq *CategoryQuery) QueryExpenses() *ExpenseQuery {
...
func (cq *CategoryQuery) QueryBudgets() *BudgetQuery {
	query := (&BudgetClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryRecurringExpenses chains the current query on the "recurring_expenses" edge.
func (cq *CategoryQuery) QueryRecurringExpenses() *RecurringExpenseQuery {
	query := (&RecurringExpenseClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withBudgets:           cq.withBudgets.Clone(),
		withRecurringExpenses: cq.withRecurringExpenses.Clone(),
...
func (cq *CategoryQuery) WithExpenses(opts ...func(*ExpenseQuery)) *CategoryQuery {
	query := (&ExpenseClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CategoryQuery) WithBudgets(opts ...func(*BudgetQuery)) *CategoryQuery {
	query := (&BudgetClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withBudgets = query
	return cq
}

// WithRecurringExpenses tells the query-builder to eager-load the nodes that are connected to
// the "recurring_expenses" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithRecurringExpenses(opts ...func(*RecurringExpenseQuery)) *CategoryQuery {
	query := (&RecurringExpenseClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withRecurringExpenses = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withExpenses; query != nil {
		if err := cq.loadExpenses(ctx, query, nodes,
			func(n *Category) { n.Edges.Expenses = []*Expense{} },
			func(n *Category, e *Expense) { n.Edges.Expenses = append(n.Edges.Expenses, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withBudgets; query != nil {
		if err := cq.loadBudgets(ctx, query, nodes,
			func(n *Category) { n.Edges.Budgets = []*Budget{} },
			func(n *Category, e *Budget) { n.Edges.Budgets = append(n.Edges.Budgets, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withRecurringExpenses; query != nil {
		if err := cq.loadRecurringExpenses(ctx, query, nodes,
			func(n *Category) { n.Edges.RecurringExpenses = []*RecurringExpense{} },
			func(n *Category, e *RecurringExpense) {
				n.Edges.RecurringExpenses = append(n.Edges.RecurringExpenses, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (cq *CategoryQuery) loadExpenses(ctx context.Context, query *ExpenseQuery, nodes []*Category, init func(*Category), assign func(*Category, *Expense)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(expense.FieldCategoryID)
	}
	query.Where(predicate.Expense(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ExpensesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CategoryQuery) loadBudgets(ctx context.Context, query *BudgetQuery, nodes []*Category, init func(*Category), assign func(*Category, *Budget)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(budget.FieldCategoryID)
	}
...
func (cq *CategoryQuery) loadRecurringExpenses(ctx context.Context, query *RecurringExpenseQuery, nodes []*Category, init func(*Category), assign func(*Category, *RecurringExpense)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(recurringexpense.FieldCategoryID)
	}
	query.Where(predicate.RecurringExpense(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.RecurringExpensesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
