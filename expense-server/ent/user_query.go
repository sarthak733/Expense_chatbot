...
func (uq *UserQuery) QueryExpenses() *ExpenseQuery {
	query := (&ExpenseClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryBudgets() *BudgetQuery {
	query := (&BudgetClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryRecurringExpenses() *RecurringExpenseQuery {
	query := (&RecurringExpenseClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withExpenses:          uq.withExpenses.Clone(),
		withBudgets:           uq.withBudgets.Clone(),
		withRecurringExpenses: uq.withRecurringExpenses.Clone(),
...
func (uq *UserQuery) WithExpenses(opts ...func(*ExpenseQuery)) *UserQuery {
	query := (&ExpenseClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withExpenses = query
	return uq
}
...
func (uq *UserQuery) WithBudgets(opts ...func(*BudgetQuery)) *UserQuery {
	query := (&BudgetClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withBudgets = query
	return uq
}
...
func (uq *UserQuery) WithRecurringExpenses(opts ...func(*RecurringExpenseQuery)) *UserQuery {
	query := (&RecurringExpenseClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withRecurringExpenses = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withExpenses; query != nil {
		if err := uq.loadExpenses(ctx, query, nodes,
			func(n *User) { n.Edges.Expenses = []*Expense{} },
			func(n *User, e *Expense) { n.Edges.Expenses = append(n.Edges.Expenses, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withBudgets; query != nil {
		if err := uq.loadBudgets(ctx, query, nodes,
			func(n *User) { n.Edges.Budgets = []*Budget{} },
			func(n *User, e *Budget) { n.Edges.Budgets = append(n.Edges.Budgets, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withRecurringExpenses; query != nil {
		if err := uq.loadRecurringExpenses(ctx, query, nodes,
			func(n *User) { n.Edges.RecurringExpenses = []*RecurringExpense{} },
			func(n *User, e *RecurringExpense) { n.Edges.RecurringExpenses = append(n.Edges.RecurringExpenses, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadExpenses(ctx context.Context, query *ExpenseQuery, nodes []*User, init func(*User), assign func(*User, *Expense)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(expense.FieldUserID)
	}
...
func (uq *UserQuery) loadBudgets(ctx context.Context, query *BudgetQuery, nodes []*User, init func(*User), assign func(*User, *Budget)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(budget.FieldUserID)
	}
...
func (uq *UserQuery) loadRecurringExpenses(ctx context.Context, query *RecurringExpenseQuery, nodes []*User, init func(*User), assign func(*User, *RecurringExpense)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(recurringexpense.FieldUserID)
	}
	query.Where(predicate.RecurringExpense(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RecurringExpensesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
